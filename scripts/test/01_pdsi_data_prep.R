...
# read in the Palmer Drought Severity Index (pdsi) data as a raster 
# scale of 0 -10 
# want to get average number of days a year with at least moderate drought (<=-2)
projection <- "epsg:5070"

pdsi_rst <- rast(here::here("data/original/clim_lab/agg_met_pdsi_1979_CurrentYear_CONUS.nc"))
pdsi_rst
test <- pdsi_rst["category"]
test

test_slice <- test[[2483:3213]]
test_slice
pdsi_rst <- as(pdsi_rst, "SpatRaster")
#plot(pdsi_rst)

pdsi_rst_proj <- project(test_slice, projection)
pdsi_rst_proj
names(pdsi_rst_proj)
...
r.drought <- reclassify(brick(pdsi_rst_proj), rclmat)
r.drought
plot(sum(r.drought))
test_final <- rast(sum(r.drought))
test_final
...
nc_data <- nc_open(here::here("data/original/clim_lab/agg_met_pdsi_1979_CurrentYear_CONUS.nc"))
# Save the print(nc) dump to a text file
{
  sink(here::here("data/original/clim_lab/agg_met_pdsi_1979_CurrentYear_CONUS.txt"))
  print(nc_data)
  sink()
}
